...
export default function HubScene() {
  ...
  const instIconMap: Record<string, string>  = { song_wang: '⛵', gan_ku_office: '🏢', tribunal: '⚖️', castle: '🏰' };
  const instColorMap: Record<string, string> = { song_wang: 'rgba(220,160,80,.65)', gan_ku_office: 'rgba(100,160,220,.6)', tribunal: 'rgba(160,130,255,.6)', castle: 'rgba(100,200,155,.5)' };
  ...
  return (
    <div ref={rootRef} style={{
      ...
      <div style={{
        ...
        {[
          { id:'instance', icon:'✲', title:'進入副本',   ac:'rgba(220,160,80,.6)',
            desc:`選擇記憶碎片 · 進入指定世界\n已開放 ${(hubConfig.instances as HubInstanceEntry[]).length} 個副本` },
          { id:'backpack', icon:'◈',  title:'遺物背包',   ac:'rgba(160,130,255,.6)',
            desc:`永久遺物 · 副本遺物 · 詳細查閱\n當前持有：${globalInventory.length} 件永久遺物` },
          { id:'achieve',  icon:'◎',  title:'輪迴紀錄',   ac:'rgba(200,180,100,.6)',
            desc:`成就 · 結局紀錄 · 命格歷史\n已解鎖 ${ACHIEVEMENTS.filter(a => globalTags.includes(a.req_global_tag)).length} / ${ACHIEVEMENTS.length} 項` },
      ...
      <div style={{
        ...
        {(() => {
          const lastEntry = Object.entries(clearedInstances)
            .sort((a, b) => b[1].clearedAt.localeCompare(a[1].clearedAt))[0];
          const lastInstName = lastEntry
            ? (hubConfig.instances as HubInstanceEntry[]).find(i => i.id === lastEntry[0])?.name ?? lastEntry[0]
            : '—';
          const lastEnding = lastEntry ? lastEntry[1].ending : '—';
          return (
            <>
              <div style={{display:'flex',alignItems:'center',gap:8,padding:'0 20px',
                ...
                <span style={{color:'rgba(200,185,240,.55)',letterSpacing:'.1em'}}>{lastInstName}</span>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:8,padding:'0 20px',
                borderRight:'1px solid rgba(180,165,220,.08)',height:'100%'}}>
                <span style={{color:'rgba(160,145,200,.3)',letterSpacing:'.15em'}}>上次結局</span>
                <span style={{color:'rgba(200,180,100,.65)',letterSpacing:'.1em'}}>{lastEnding}</span>
              </div>
            </>
          );
        })()}
            ...
            <>
              ...
              <Body>
                {(hubConfig.instances as HubInstanceEntry[]).map(inst => {
                  ...
                  const record = clearedInstances[inst.id];
                  return (
                    <div key={inst.id} style={{
                      ...
                      <div style={{flex:1}}>
                        ...
                        <div style={{fontFamily:M.mono,fontSize:8,letterSpacing:'.08em',color:'rgba(160,145,200,.38)',lineHeight:1.6}}>
                          {ok ? (record ? `已通關 · ${record.ending}` : '已解鎖 · 尚未通關') : `解鎖條件：${inst.unlock_condition?.req_global_tag ?? inst.unlock_condition?.req_global_item ?? '未知'}`}
                        </div>
                      </div>
                      <div style={{textAlign:'right',flexShrink:0}}>
                        <div style={{fontFamily:M.mono,fontSize:8.5,letterSpacing:'.1em',color: record ? 'rgba(200,180,60,.7)' : ac}}>
                          {record ? record.ending.split(' · ')[0] : (ok ? '已解鎖' : '▶ 待解中')}
                        </div>
                        {ok && (
            ...
            <>
              ...
              <Body>
                <div style={{marginBottom:18}}>
                  <SecTitle>GLOBAL · 永久持有</SecTitle>
                  {globalInventory.length === 0 ? (
                    <div style={{fontFamily:M.mono,fontSize:9,color:'rgba(160,145,200,.3)'}}>尚無永久遺物</div>
                  ) : (
                    <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:8}}>
                      {globalInventory.map(itemId => {
                        const def = itemRegistry[itemId];
                        const rarityColorMap: Record<string,string> = {
                          EPIC:'rgba(160,130,255,.55)',LEGENDARY:'rgba(220,180,60,.6)',
                          RARE:'rgba(80,140,220,.52)',CURSED:'rgba(220,80,80,.55)',COMMON:'rgba(160,148,185,.4)',
                        };
                        const rc = rarityColorMap[def?.rarity ?? ''] ?? 'rgba(160,148,185,.4)';
                        return (
                          <div key={itemId} style={{
                            ...
                            <span style={{fontSize:19}}>{def?.icon_emoji ?? '📦'}</span>
                            <span style={{fontFamily:M.mono,fontSize:7,color:'rgba(160,148,195,.4)',letterSpacing:'.04em',textAlign:'center',padding:'0 2px'}}>{def?.name ?? itemId}</span>
                            <span style={{position:'absolute',top:2,right:3,fontFamily:M.mono,fontSize:6.5,color:rc}}>{def?.rarity ?? 'CMN'}</span>
                          </div>
                        );
                      })}
                      {Array(Math.max(0, 6 - (globalInventory.length % 6 || 6))).fill(null).map((_,i) => (
                        <div key={`eg${i}`} style={{aspectRatio:'1',border:'1px solid rgba(180,165,220,.08)',background:'rgba(12,10,24,.55)',opacity:.2}}/>
                      ))}
                    </div>
                  )}
                </div>
                <div>
                  <SecTitle>LOCAL · 副本遺物</SecTitle>
                  <div style={{fontFamily:M.mono,fontSize:9,color:'rgba(160,145,200,.28)'}}>
                    副本內道具於副本結束後歸零，不帶出
                  </div>
                </div>
              </Body>
            ...
            <>
              ...
              <Body>
                {ACHIEVEMENTS.map(ach => {
                  const unlocked = globalTags.includes(ach.req_global_tag);
                  return (
                    <div key={ach.id} style={{
                      display:'flex',alignItems:'center',gap:14,padding:'12px 0',
                      borderBottom:'1px solid rgba(180,165,220,.05)',opacity: unlocked ? 1 : .25,
                    }}>
                      <div style={{width:36,height:36,border:'1px solid rgba(180,165,220,.1)',
                        display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0}}>{ach.icon}</div>
                      <div style={{flex:1}}>
                        <div style={{fontSize:13.5,color:'rgba(210,200,240,.62)',marginBottom:3}}>{ach.name}</div>
                        <div style={{fontFamily:M.mono,fontSize:8,letterSpacing:'.08em',color:'rgba(150,138,185,.34)',lineHeight:1.55}}>{ach.desc}</div>
                      </div>
                      <div style={{fontFamily:M.mono,fontSize:8.5,flexShrink:0,marginLeft:'auto',
                        color: unlocked ? 'rgba(200,180,60,.7)' : 'rgba(160,145,200,.25)'}}>
                        {unlocked ? ach.unlocked_label : '封印'}
                      </div>
                    </div>
                  );
                })}
          ...
          {panel === 'settings' && (
            <>
              ...
              <Body>
                ...
                <div style={{marginTop:20,paddingTop:16,borderTop:'1px solid rgba(180,165,220,.06)'}}>
                  <div style={{fontSize:13.5,color:'rgba(200,190,230,.58)',marginBottom:4}}>清除存檔</div>
                  <div style={{fontFamily:M.mono,fontSize:8,letterSpacing:'.1em',color:'rgba(150,138,185,.3)',marginBottom:10}}>
                    刪除所有全域進度、成就、副本紀錄，無法復原
                  </div>
                  <button
                    onClick={() => { if (window.confirm('確定要清除所有存檔嗎？')) resetSave(); }}
                    style={{
                      padding:'6px 14px',background:'transparent',
                      border:'1px solid rgba(220,80,80,.25)',
                      fontFamily:M.mono,fontSize:8.5,letterSpacing:'.15em',
                      color:'rgba(220,100,100,.5)',cursor:'pointer',transition:'all .2s',
                    }}
                  >
                    清除存檔
                  </button>
